...
;; Created:2008-10-01
;; Updated: Time-stamp: <2015-10-19 13:47:57>
;; --8<-------------------------- separator ------------------------>8--
(defun scratch ()
  (interactive)
...
Open windows explorer in the current directory"
  ...
  (cond
   ((eq system-type 'gnu/linux)
    (shell-command-to-string (format "nautilus \"%s\"" (file-truename default-directory))))
   ((eq system-type 'darwin)
    (shell-command-to-string (format "open \"%s\"" (file-truename default-directory))))
...
(defun ramfs-flush()
  (interactive)
  (rsync-dir
   (concat SHARE_DIR "private_data/emacs_stuff/org_data/")
   (concat SHARE_DIR "private_data/emacs_stuff/org_data.disk"))
  )
;; --8<-------------------------- separator ------------------------>8--
