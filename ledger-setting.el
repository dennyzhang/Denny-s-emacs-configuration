...
statement for insertion to sqlite db"
  ...
  (with-current-buffer
    (let ((output-buffername "*ledger http*")
      ...
      (find-file (concat CONF-DENNY-EMACS "/bank/filebat.ledger"))
...
(defun ledger-transactions (&optional record-date ledger-file-name)
 ...
 "
  (let ((ledger-buffer (concat CONF-DENNY-EMACS "/bank/filebat.ledger"))
...
(defun ledger-generate-transaction-http-call(&optional out-buffname ledger-file-name)
  ...
  (interactive)
  (let ((output-buffername "*ledger http*")
        (ledger-buffer (concat CONF-DENNY-EMACS "/bank/filebat.ledger"))
        http-request-str
        transaction-list
